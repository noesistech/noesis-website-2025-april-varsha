      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-gray-300">
            Meet the talented individuals who make our company exceptional. We're a diverse team of 
            dedicated professionals committed to delivering excellence in everything we do.
          </p>
        </div>
            ...
            <Accordion type="single" collapsible className="w-full max-w-md">
              <AccordionItem value="filter">
                <AccordionTrigger className="text-white">
                  Filter by Department: <span className="text-noesis-purple ml-2">{selectedCategory}</span>
                </AccordionTrigger>
                <AccordionContent>
                  <ScrollArea className="h-[200px] w-full rounded-md px-4">
                        ...
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 rounded-lg ${
                            selectedCategory === group
                              ? 'bg-noesis-purple/80 text-white'
            ...
            >
              {departmentGroups.map(group => (
                <ToggleGroupItem 
                  key={group} 
                  value={group} 
                  className="bg-gray-800/50 text-gray-300 data-[state=on]:bg-noesis-purple/80 data-[state=on]:text-white"
                >
                  {group}
                </ToggleGroupItem>
              ))}
