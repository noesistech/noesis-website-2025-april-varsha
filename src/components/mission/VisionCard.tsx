...
const VisionCard = ({
  ...
  return <div className="relative flex flex-col items-center h-full" style={{
    animationDelay: '0.2s'
      ...
      <div className="z-20 absolute top-8 -left-6 bg-noesis-blue/15 text-noesis-blue rounded-full p-4 md:p-6 shadow-lg my-0 mx-[40px]">
      ...
      <div id="vision-card" className="glass-card animate-fade-in relative transition-transform duration-500 ease-out transform p-8 md:p-10 mt-16 pt-8 md:pt-10 w-full h-full">
        <h3 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-4 md:mb-8 relative text-left pt-8 leading-tight">
          {title}
          <span className="absolute left-0 bottom-[-15px] w-16 h-1 bg-gradient-to-r from-noesis-purple to-noesis-blue rounded-full"></span>
        </h3>
        <p className="text-standard mt-4 md:mt-8 text-left">
          {description}
        </p>
      </div>
    </div>;
