...
const applicationFormSchema = z.object({
  ...
  position: z.string().min(2, { message: "Please specify a position you're applying for." }),
  message: z.string().min(10, { message: "Cover letter should be at least 10 characters." }).max(1000, { message: "Cover letter should not exceed 1000 characters." }),
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-16">
          <h3 className="text-2xl font-semibold text-white mb-8 flex items-center">
            <Briefcase className="h-6 w-6 mr-3 text-noesis-purple" />
            Current Opportunities
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {opportunities.map((job, index) => (
              <div 
                key={index}
                className="bg-gradient-to-br from-purple-900/20 to-indigo-900/20 border border-purple-500/20 rounded-xl p-6 hover:border-purple-500/40 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/10"
              >
                <h4 className="text-xl font-semibold text-white mb-2">{job.title}</h4>
                <div className="flex items-center text-sm text-gray-400 mb-4">
                  <span className="mr-3">{job.department}</span>
                  <span className="bg-noesis-purple/30 text-white px-2 py-0.5 rounded-full text-xs">{job.type}</span>
                </div>
                <p className="text-gray-300 mb-4">{job.description}</p>
                <Button variant="noesis" size="sm" className="mt-2">Apply Now</Button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto">
          <h3 className="text-2xl font-semibold text-white mb-8 flex items-center">
            <FileText className="h-6 w-6 mr-3 text-noesis-purple" />
            General Application
          </h3>
          ...
          <div className="bg-gradient-to-br from-purple-900/10 to-indigo-900/10 border border-purple-500/20 rounded-xl p-6 md:p-8">
            <p className="text-gray-300 mb-6">
              Don't see a perfect fit? Send us your resume and let us know how you can contribute to our team.
            </p>
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          <Input 
                            ...
                            {...field} 
                            className="bg-[#242938] border-purple-500/20 focus:border-purple-500/50 text-white" 
                          />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea 
                          ...
                          {...field} 
                          className="min-h-[120px] bg-[#242938] border-purple-500/20 focus:border-purple-500/50 text-white" 
                        />
                      </FormControl>
