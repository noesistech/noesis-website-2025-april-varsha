...
const TechStackSection: React.FC<TechStackSectionProps> = ({
  ...
  return (
    <section id="tech-stack" className="py-10 sm:py-16 md:py-[40px] relative overflow-hidden">
                ...
                <div key={category.key} className="glass-card">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {category.technologies?.map(tech => (
                      <span key={tech.id} className="bg-white/10 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm text-white/80 hover:bg-noesis-purple/20 transition-colors">
                        {tech.name}
                      </span>
                    ))}
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
              {cloudStack.map(category => (
                <div key={category.key} className="glass-card">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {category.technologies?.map(tech => (
                      <span key={tech.id} className="bg-white/10 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm text-white/80 hover:bg-noesis-blue/20 transition-colors">
                        {tech.name}
                      </span>
                    ))}
                  </div>
