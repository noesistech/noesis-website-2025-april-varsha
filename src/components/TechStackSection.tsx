    ...
    <section className="page-section overflow-hidden relative">
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        <Tabs defaultValue="tech" value={activeTab} onValueChange={setActiveTab} className="max-w-6xl mx-auto">
          ...
          <TabsContent value="tech" className="animate-fade-in mt-6">
                ...
                <Card key={category.id} className="bg-[#1e1e24]/90 border-0 overflow-hidden rounded-lg p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {category.technologies?.map(item => (
                      <span key={item.id} className="bg-gray-800/80 text-white/90 px-4 py-2 text-sm rounded-full">
                        {item.name}
                      </span>
                    ))}
                  </div>
          ...
          <TabsContent value="cloud" className="animate-fade-in mt-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
